...
export default function TestDashboardPage() {
  ...
  useEffect(() => {
    if (!activeSub) return;
    setData(null);
    api.get(`/test-master/${deptCode}/dashboard?sub_department=${encodeURIComponent(activeSub)}`).then(setData).catch((e) => setError(e.message));
  }, [deptCode, activeSub]);

  if (error) return <div className="content"><div className="card error-text">{error}</div></div>;
  if (!subDepts.length || !data) return <div className="content">Loading...</div>;

  const topTests = [...data.tests].sort((a, b) => b.total_actual - a.total_actual).slice(0, 10);
  const ov = data.overhead || {};
  const overheadPie = [
    { name: "Manpower", value: ov.manpower_annual_total || 0 },
    { name: "Equipment", value: ov.equipment_annual_total || 0 },
    { name: "Building", value: ov.building_annual_total || 0 },
    { name: "Power", value: ov.power_annual_total || 0 },
    { name: "Common Consumables", value: ov.common_consumables_annual_total || 0 },
  ].filter((d) => d.value > 0);
    ...
    <div className="content">
      <div className="card">
        ...
        <div className="grid grid-4">
          ...
          <div className="metric"><p className="metric-label">Overhead / test (actual)</p><p className="metric-value">₹{(ov.overhead_per_test_actual || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}</p></div>
        </div>
      </div>
      ...
      <div className="grid grid-4" style={{ gridTemplateColumns: "1fr 1.4fr" }}>
        {overheadPie.length > 0 && (
          <div className="card">
            <p className="card-title">Overhead composition ({activeSub}, annual)</p>
        ...
        <div className="card">
          <p className="card-title">Top 10 most expensive tests in {activeSub} (actual)</p>
